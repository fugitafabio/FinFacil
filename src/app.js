...
const rowsToProducts = rows => rows.slice(1).map(r=>({ id:Number(r[0]), name:r[1]||"", cat:r[2]||"Produto", price:parseFloat(r[3])||0, desc:r[4]||"" }));
const rowsToSales = rows => rows.slice(1).map(r=>({ id:Number(r[0]), productId:Number(r[1]), qty:Number(r[2]), date:r[3]||"", note:r[4]||"", month:Number(r[5]), year:Number(r[6]) }));
const rowsToExpenses = rows => rows.slice(1).map(r=>({ id:Number(r[0]), desc:r[1]||"", cat:r[2]||"outro", value:parseFloat(r[3])||0, date:r[4]||"", month:Number(r[5]), year:Number(r[6]) }));
...
const Inp = ({label,type="text",value,onChange,placeholder,hint,prefix}) => (
...
export default function App() {
  ...
  const loadAll=async(tk)=>{
    ...
    try {
      ...
      const [pr,sr,er]=await Promise.all([sheetsGet(tk,"Produtos!A:E"),sheetsGet(tk,"Vendas!A:G"),sheetsGet(tk,"Despesas!A:G")]);
      ...
      setLoadMsg("");
    } catch(e){ setLoadMsg("Erro: "+e.message); }
  };

  const ensureHeaders=async(tk)=>{
    const check=async(range,headers)=>{ const rows=await sheetsGet(tk,range+"!A1:Z1"); if(!rows.length||!rows[0].length) await sheetsWrite(tk,range+"!A1",[headers]); };
  ...
  const saveProducts=useCallback(async(data)=>{ if(!token)return; setSaving(true); try{ await sheetsClear(token,"Produtos!A:E"); await sheetsWrite(token,"Produtos!A1",[["id","nome","tipo","preco","descricao"],...data.map(p=>[p.id,p.name,p.cat,p.price,p.desc])]); }finally{ setSaving(false); } },[token]);
  const saveSales=useCallback(async(data)=>{ if(!token)return; setSaving(true); try{ await sheetsClear(token,"Vendas!A:G"); await sheetsWrite(token,"Vendas!A1",[["id","productId","qty","data","nota","mes","ano"],...data.map(s=>[s.id,s.productId,s.qty,s.date,s.note,s.month,s.year])]); }finally{ setSaving(false); } },[token]);
  const saveExpenses=useCallback(async(data)=>{ if(!token)return; setSaving(true); try{ await sheetsClear(token,"Despesas!A:G"); await sheetsWrite(token,"Despesas!A1",[["id","descricao","categoria","valor","data","mes","ano"],...data.map(e=>[e.id,e.desc,e.cat,e.value,e.date,e.month,e.year])]); }finally{ setSaving(false); } },[token]);
...
function Dashboard({products,filteredSales,filteredExpenses,totalRevenue,totalExpense,profit,margin,fmt,sales,expenses,selYear}){
  ...
  return <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
      {[["💰","Receita",fmt(totalRevenue),"#10b981","Soma de todas as vendas"],["💸","Despesas",fmt(totalExpense),"#ef4444","Total de custos"],["📈","Lucro",fmt(profit),profit>=0?"#6366f1":"#ef4444","Receita menos despesas"],["%","Margem",`${margin}%`,"#f59e0b","% de lucro sobre receita"]].map(([icon,label,val,color,tip])=>(
        <TT key={label} text={tip}>
          <div style={{background:"#fff",borderRadius:14,padding:16,boxShadow:"0 2px 10px rgba(0,0,0,.07)",borderLeft:`4px solid ${color}`,cursor:"default"}}>
...
function Sales({sales,setSales,products,filteredSales,fmt}){
  ...
  return <div>
    ...
    <Card>
      ...
      <Inp label="Observação" value={form.note} onChange={v=>setForm({...form,note:v})} placeholder="Ex: Pix, cartão..."/>
      <div style={{display:"flex",gap:8}}>
...
function Reports({products,sales,expenses,fmt,selYear}){
  ...
  return <div>
    ...
    <Card>
      ...
      <p style={{color:"#6b7280",fontSize:14,margin:"0 0 8px"}}>Mínimo para cobrir os custos fixos.</p>
